      ...
      case 'P':
      {
	    ...
	    for (int i = user_num() - 1; i >= 0; i--)
	    {
	        if(list[i].card !=18072801&&list[i].balance!=0)
	        {
	           t++;
		       printf("\t├──────┼───────────────┼───────────┼────────────┤\n");
		       printf("\t│%6d│%15s│%11d│%12.2f│\n", t, list[i].name, list[i].card, list[i].balance);
	        }
	    }  
	     printf("\t└──────┴───────────────┴───────────┴────────────┘\n");
	    printf("\t\t$$共有%d条数据\n",t);
...
void handle_save(struct tm *t, float money,char arr[10])
{
	...
	p->list[num_4].T = *t;   //保存时间 
	p->list[num_4].money = money;   //保存操作的钱数 
	file_read(p1);  //读取文件 
