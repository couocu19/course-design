...
void account()
{
	...
	if (strcmp(list[i].name, p->name) == 0&&list[i].card==p->card)
	{
		printf("\t\t├─────┼─────────────────┼─────────────┤\n");
...
//{
//	return (((user*)x)->balance-((user*)y)->balance);
//}
//用户列表，管理员  /////
void user_list()
{
...
void user_tj()
{
	...
	char h1;
	user list[user_max] = { NULL };
	printf("欢迎管理员%s\t卡号：%ld\n","coucou",18072801);
    printf("\n\n");
    printf("\t键入选择：(P/S/回车)\n");
    printf("\t\t\tP.有余额用户 \t\t\tS.无余额用户 \t\t\t按回车键返回…… \n");
    h1=getchar();
    switch(h1)
    {
      case 'P':
      {
	    ...
	    {
	    	for(j=0;j<user_num()-1-i;j++)
	    	{
	    		if(list[j].balance==0)
	    		  continue;
	    		else 
	    		{
	    			if(list[j].balance>list[j+1].balance)
	    			{
	    				L=list[j];
	    				list[j]=list[j+1];
	    				list[j+1]=L;
	    ...
	    printf("\n\n");
	    printf("\t\t有余额用户列表如下：\n\n");
		printf("\t\t【提示】系统默认按照存款由多到少排序哦~\n\n"); 
	    printf("\t┌──────┬───────────────┬───────────┬────────────┐\n");
	    printf("\t│序号  │姓名           │卡号       │余额        │\n");
	    for (int i = user_num() - 1; i >= 0; i--)
	    {
	        if(list[i].card !=18072801&&list[i].balance!=0)
	        {
	           t++;
		       printf("\t├──────┼───────────────┼───────────┼────────────┤\n");
		       printf("\t│%6d│%15s│%11d│%12.2f│\n", i, list[i].name, list[i].card, list[i].balance);
...
void file_save(user *p, int count) 
{
	...
	//指向保存数据的指针；size：每个数据类型的大小；count：数据的个数；stream：文件指针
	fclose(data); //fwrite()使用之后必须使用fclose()！ 
}
//读取文件 ///
void file_read(user *p)
{
